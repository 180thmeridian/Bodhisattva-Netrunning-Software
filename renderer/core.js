...
function log(msg,cls=''){
  ...
  if(msgLevel < cur && cls !== 'sys' && cls !== 'bad' && cls !== 'error') return;
...
function updateRunnerBars(){
  document.getElementById('nr-wounds').textContent=S.wounds;
  document.getElementById('nr-wounds').style.color=S.wounds>0?'var(--r)':'var(--g)';
  document.getElementById('nr-intdmg').textContent=S.intDmg;
  document.getElementById('nr-intdmg').style.color=S.intDmg>0?'var(--r)':'var(--g)';
  const b=[];
  ...
  document.getElementById('nr-buffs').textContent=b.length?b.join(', '):'—';
}
function renderPrograms(){
  ...
  S.programs.forEach((p,i)=>{
    ...
    div.className='prog'+(i===S.selectedProg?' sel':'');
    let meta=`${p.cls} · STR ${p.str} · MU ${p.mu}`;
    if(isDemon(p)) meta+=` · slots ${p.slots.length}/${demonSlotMax(p)}`;
    let slotHtml='';
    if(isDemon(p) && p.slots.length){
      slotHtml='<div class="meta" style="margin-top:3px;color:#33ccff">'+p.slots.map((s,si)=>
        `↳ ${s.name} <button data-unload="${i}:${si}" class="amber" style="padding:0 4px;font-size:9px">⏏</button>`
      ).join('<br/>')+'</div>';
    }
    const selDemon=S.programs[S.selectedProg];
    const showLoad = selDemon && isDemon(selDemon) && !isDemon(p) && i!==S.selectedProg;
    div.innerHTML=`<div><div class="name">${p.name}${isDemon(p)?' ◈':''}</div><div class="meta">${meta}</div>${slotHtml}</div>
...
window.key = key; window.log = log; window.nr = nr; window.deck = deck;
